...
sys.path.insert(0, '/Users/eduardamarques/eixo-eleicoes')
from compartilhado import pollingdata_scraper as ps

S = '/private/tmp/claude-501/-Users-eduardamarques/17b39da5-495c-48d0-bd21-f97e17ed765d/scratchpad/'
pesq = pd.read_parquet(S + 'cache_pesquisas.parquet')
res = pd.read_parquet(S + 'cache_resultados.parquet')
bi = pd.read_parquet(S + 'cache_resultados_bi.parquet')

gov_r = res[(res.cargo == 'governador') & (res.turno == 't1')].copy()
gov_r['percentual'] = pd.to_numeric(gov_r.percentual, errors='coerce')
...
bi_gov = bi[(bi.cargo == 'governador') & (bi.turno == 't1')].copy()
...
for uf in sorted(nomes_uf.keys()):
    ...
    lider_nome_bi = vivos_bi.iloc[0]['candidato_partido'] if len(vivos_bi) > 0 else ''
    ...
    delta_lider = p1_lider_pct_val - p2_lider_pct_val if p1_lider_nome == p2_lider_nome else p1_lider_pct_val - p2_cands[p2_cands.candidato_partido == p1_lider_nome]['percentual'].sum()
    
    n1 = p1_lider_nome.split()[0] if p1_lider_nome != '-' else ''
    n2 = p2_lider_nome.split()[0] if p2_lider_nome != '-' else ''
    n_bi = lider_nome_bi.split()[0] if lider_nome_bi else ''
    
    # Diagnóstico neutro
    if p1_lider_nome != p2_lider_nome and p2_lider_nome != '-':
        obs = f"Líderes diferentes entre as pesquisas: {p1_inst} aponta {n1} e {p2_inst} aponta {n2}"
        divergentes.append(f"{uf} ({n1} x {n2})")
    elif p1_lider_nome != lider_nome_bi and p2_lider_nome != lider_nome_bi:
        obs = f"Líder recente diferente da Média Eixo: pesquisas apontam {n1}, enquanto Média projeta {n_bi}"
        ...
        obs = "Levantamentos indicam probabilidade de definição em 1º turno"
...
# Row 0: Title
grid.append(['ÚLTIMAS PESQUISAS - COMPARATIVO ENTRE LEVANTAMENTOS RECENTES (SETEMBRO/2026)'] + [''] * (N_COLS - 1))
# Row 1: Subtitle
...
grid.append([
    'LÍDERES DIFERENTES ENTRE AS PESQUISAS (6 UFs)', '', '', '', '',
    'VARIAÇÃO ACIMA DE 6 p.p. (7 UFs)', '', '', '', '', '',
    'MARGEM ESTREITA / EMPATE', '', '', '', '',
    'PROJEÇÃO DE 1º TURNO', '', '', '', '', ''
])

# Row 6: Cards Detail
c1_txt = "Líderes diferentes entre as pesquisas: " + ", ".join(divergentes)
c2_txt = "Variação > 6 p.p. no líder: " + ", ".join(altas_disparidades)
c3_txt = "Diferença < 1.5 p.p.: AM (Omar Aziz x Roberto Cidade)"
c4_txt = "Líder > 50% em ambas: " + ", ".join(primeiro_turno)
grid.append([
    c1_txt, '', '', '', '',
    c2_txt, '', '', '', '', '',
    c3_txt, '', '', '', '',
...
grid.append(['Confronto dos principais candidatos nos estados onde os levantamentos apontam cenários distintos ou margens competitivas.'] + [''] * (N_COLS - 1))
grid.append([''] * N_COLS)

estados_foco = ['BA', 'CE', 'ES', 'MT', 'TO', 'PA', 'RJ', 'RS', 'SP']

observacoes_foco = {
    'BA': 'Observação: Real Time Big Data indica Jerônimo Rodrigues em 1º lugar, enquanto a AtlasIntel aponta ACM Neto à frente. Na Média Ponderada Eixo, ACM Neto mantém a liderança.',
    'CE': 'Observação: Instituto Veritá indica Ciro Gomes em 1º lugar, enquanto a Real Time Big Data aponta Elmano de Freitas. Na Média Ponderada Eixo, Ciro Gomes permanece na dianteira.',
    'ES': 'Observação: Real Time Big Data projeta Ricardo Ferraço em 1º lugar e a AtlasIntel aponta Lorenzo Pazolini. Na Média Ponderada Eixo, Ferraço mantém a liderança.',
    'MT': 'Observação: AtlasIntel indica Wellington Fagundes em 1º lugar e o Paraná Pesquisas aponta Otaviano Pivetta. Na Média Ponderada Eixo, Wellington lidera.',
    'TO': 'Observação: Levantamentos mais recentes (Correio do Povo e O Girassol) apontam Vicentinho Júnior em 1º lugar. Na Média Ponderada Eixo, Dorinha Seabra conserva a liderança devido ao peso dos levantamentos anteriores de institutos de maior nota histórica (Paraná Pesquisas e Quaest).',
    'PA': 'Observação: Levantamentos de Doxa e AtlasIntel colocam Hana Ghassan à frente de Daniel Santos. Na Média Ponderada Eixo, Daniel Santos mantém a dianteira pelo acumulado dos últimos 30 dias.',
    'RJ': 'Observação: Ambos os levantamentos indicam Eduardo Paes na liderança sobre Douglas Ruas, registrando variação na amplitude da vantagem entre os institutos.',
    'RS': 'Observação: Levantamentos indicam proximidade entre Tenente Coronel Zucco e Juliana Brizola, com Gabriel Souza ocupando a 3ª posição.',
    'SP': 'Observação: Ambos os levantamentos indicam Tarcísio de Freitas à frente de Fernando Haddad, com a Real Time Big Data projetando margem suficiente para encerramento em 1º turno.'
}
...
for uf in estados_foco:
    ...
    # Linha de Observação
    r_diag = len(grid)
    diag_txt = observacoes_foco.get(uf, 'Observação: Levantamentos demonstram estabilidade ou variações dentro da margem de erro.')
...
print(f"Grid construído: {len(grid)} linhas x {N_COLS} colunas")

# Conectar ao Google Sheets
creds = Credentials.from_service_account_file('/Users/eduardamarques/eixo-eleicoes/credentials.json',
                                              scopes=['https://www.googleapis.com/auth/spreadsheets'])
gc = gspread.authorize(creds)
gc.timeout = 60
sh = gc.open_by_key('1V1JvSjea_G8ZTemAE3ZyJxcAc2RLwNfzOC7WGGlb6PI')
...
print("Valores inseridos. Gerando formatação profissional...")
...
for info in detalhe_info_estados:
    ...
    bg_diag = C['alerta_bg'] if (info['uf'] in ['BA', 'CE', 'ES', 'MT', 'TO', 'PA']) else C['gelo']
    fg_diag = C['alerta_fg'] if (info['uf'] in ['BA', 'CE', 'ES', 'MT', 'TO', 'PA']) else C['marinho']
    requests.append({'repeatCell': {
...
print(f"Total de {len(requests)} requisições de formatação. Aplicando em lotes de 40...")
for i in range(0, len(requests), 40):
    batch = requests[i:i+40]
    sh.batch_update({'requests': batch})
    time.sleep(1)

print("SUCESSO! Nova versão neutra, sem valores grudados nos nomes e sem repetição de números criada com perfeição!")
